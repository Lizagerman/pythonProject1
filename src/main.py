...
# Предполагаем, что эти функции (load_transactions_from_json, load_transactions_from_csv,
# load_transactions_from_xlsx, filter_by_status, sort_transactions_by_date,
# filter_by_currency, format_transaction) уже реализованы в других модулях или будут реализованы
...
def filter_by_currency(transactions: List[Dict[str, Any]], currency_code: str = "RUB") -> List[Dict[str, Any]]:
    ...
    return [t for t in transactions if t.get("operationAmount", {}).get("currency", {}).get("code", "").upper() == currency_code.upper()]

def format_transaction(transaction: Dict[str, Any]) -> str:
    """Форматирует информацию о транзакции для вывода."""
    date_str = ""
    try:
        date_obj = datetime.strptime(transaction.get("date", ""), "%Y-%m-%dT%H:%M:%S.%f")
    ...
    if from_account:
        if len(from_account) > 4:
            from_account = f"{from_account[:-10]} **{from_account[-4:]}" # Пример маскировки
        else:
...
def main() -> None:
    ...
    filtered_transactions = list(transactions) # Создаем копию для дальнейшей фильтрации
    ...
    while True:
        ...
        if status_input.upper() in available_statuses:
            filtered_transactions = filter_by_status(filtered_transactions, status_input)
    ...
    # Сортировка по дате
    sort_by_date_input = input("Программа: Отсортировать операции по дате? Да/Нет\nПользователь: ")
    if sort_by_date_input.lower() == "да":
        while True:
            sort_order_input = input("Программа: Отсортировать по возрастанию или по убыванию?\nПользователь: ")
            if sort_order_input.lower() == "по возрастанию":
                ...
                print("Программа: Некорректный ввод. Пожалуйста, введите 'по возрастанию' или 'по убыванию'.")

    # Фильтрация по рублевым транзакциям
    rub_only_input = input("Программа: Выводить только рублевые транзакции? Да/Нет\nПользователь: ")
    ...
    filter_by_description_input = input("Программа: Отфильтровать список транзакций по определенному слову в описании? Да/Нет\nПользователь: ")
