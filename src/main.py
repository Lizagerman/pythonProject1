...
def test_get_mask_card_number(card_number_data: List[Tuple[str, str]]) -> None:
    ...
    for card_number, expected in card_number_data:
        assert get_mask_card_number(card_number) == expected, f"Expected {expected} but got {get_mask_card_number(card_number)} for card number {card_number}"
...
def test_filter_by_state() -> None:
    ...
    expected = [{'name': 'item1', 'state': 'active'}, {'name': 'item3', 'state': 'active'}]
    assert result == expected, f"Expected {expected} but got {result} for state 'active'"

    # Тест на фильтрацию неактивных элементов
    result = filter_by_state(data, 'inactive')
    expected = [{'name': 'item2', 'state': 'inactive'}, {'name': 'item4', 'state': 'inactive'}]
    assert result == expected, f"Expected {expected} but got {result} for state 'inactive'"

    # Тест на фильтрацию с состоянием, которого нет
    result = filter_by_state(data, 'unknown')
    expected = []
    assert result == expected, f"Expected {expected} but got {result} for state 'unknown'"
